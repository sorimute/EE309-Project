...
function App() {
  ...
  const [showShapeSettingsMenu, setShowShapeSettingsMenu] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (settingsMenuRef.current && !settingsMenuRef.current.contains(event.target as Node)) {
        setShowShapeSettingsMenu(false);
      }
    };

    if (showShapeMenu || showShapeSettingsMenu) {
  ...
  }, [showShapeMenu, showShapeSettingsMenu]);
  ...
  const moveShape = (direction: "up" | "down" | "left" | "right", distance: number = 10) => {
